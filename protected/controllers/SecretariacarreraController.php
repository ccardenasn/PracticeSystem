...
class SecretariacarreraController extends Controller
{
	...
	public function actionCreate()
	{
		...
		$emptyCarrera = isEmpty("carrera");
		
		if($emptyCarrera == false){
			...
			Yii::app()->user->setFlash('message',"<div id='errorMessage' class='flash-error'><p><strong>¡Advertencia!</strong></p><ul><li>No se puede añadir Secretaria de Carrera en este momento.</li><li>Por favor verifique que se ha agregado información de <strong>Carrera</strong>.</li></ul></div>");
			$this->redirect(array('index'));
		}
	}
		...
		if(isset($_POST['Secretariacarrera']))
		{
			...
			if($model->save()){
				if($file != null){
					...
					saveImagePath($table,$imageAttrib,$oldImage,$codTable,$model->RutSecretaria);
				}
				$this->redirect(array('view','id'=>$model->RutSecretaria));
			}
		}

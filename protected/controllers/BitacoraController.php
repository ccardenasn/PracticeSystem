...
class BitacoraController extends Controller
{
	...
	public function actionCreate() {
         ...
         */
        $modelA = new Bitacora();
        $modelB = array(new Clasebitacora);
 
        if (isset($_POST['Bitacora'])) {
            $modelA->attributes = $_POST['Bitacora'];
            ...
            if (isset($_POST['Clasebitacora'])) {
                $modelB = array();
                foreach ($_POST['Clasebitacora'] as $key => $value) {
                    /*
                     ...
                     */
                    $clase = new Clasebitacora('batchSave');
                    $clase->attributes = $value;
                    $modelB[] = $clase;
                }
            }
            /**
            ...
            foreach ($modelB as $clase) {
                $valid = $clase->validate() & $valid;
            }
 
            if ($valid) {
                $transaction = $modelA->getDbConnection()->beginTransaction();
                try {
                    $modelA->save();
                    $modelA->refresh();
 
                    foreach ($modelB as $clase) {
                        $clase->Bitacora_CodBitacora = $modelA->CodBitacora;
                        $clase->save();
                    }
                    $transaction->commit();
                } catch (Exception $e) {
                ...
                $this->redirect(array('view', 'id' => $modelA->CodBitacora));
            }
        }
        $this->render('create', array(
            'modelA' => $modelA,
